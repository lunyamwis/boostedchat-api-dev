...
class SendContent(object):
    def __init__(self, status: str, thread: Thread, generated_response: str) -> None:
        self.status = status
        self.thread = thread
        self.generated_response = generated_response

    def change_status(self, status_level, status_name):
        self.thread.replied = True
        self.thread.replied_at = timezone.now()
    ...
    def send_first_compliment(self):
        send_message.delay(self.generated_response, thread_id=self.thread.thread_id)
    ...
    def send_first_needs_assessment_question(self):
        send_message.delay(
            f"""
            {self.generated_response}
            besides I would like to notify you that,
            Seems like you are starting a great career, {self.thread.account.igname} 🔥
    ...
    def send_second_needs_assesment_question(self):
        send_message.delay(
            f"""
            {self.generated_response}
    ...
    def send_third_needs_assessment_question(self):
        send_message.delay(
            f"""
            {self.generated_response}
            by the way,
    ...
    def send_follow_up_after_presentation(self):
        send_message.delay(
            f"""
            {self.generated_response}
    ...
    def send_request_for_email(self):
        send_message.delay(
            f"""
            {self.generated_response}
    ...
    def get_reason_why_uninterested(self):
        send_message.delay(
            self.generated_response,
            thread_id=self.thread.thread_id,
        )
