    ...
    for lead in leads:
        ...
        if lead_messages[0].text != lead_saved_messages[0].content:
            ...
            if sent_by == "Lead":
                send_gpt_message(
                    lead=lead,
                    lead_saved_messages=lead_saved_messages,
                    respondent_message=lead_messages[0].text,
                    cl=None,
                )
...
def simulate_check_new_messages(respondent_message, lead: InstaLead):
    ...
    lead_saved_messages = InstaMessage.objects.filter(lead_id=lead.id).order_by("-sent_on")
    send_gpt_message(lead=lead, lead_saved_messages=lead_saved_messages, respondent_message=respondent_message, cl=None)
    serializer = InstagramMessageSerializer(data=message_data)
    if serializer.is_valid():
        serializer.save()
